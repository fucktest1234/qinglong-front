...
import CronModal from './modal'; //新建定时任务窗口
import CronLogModal from './logModal'; // 日志窗口
import RawFileModal from './RawFileModal'; // 添加raw文件窗口
...
const Crontab = ({ headerStyle, isPhone }: any) => {
  ...
  const [isModalVisible, setIsModalVisible] = useState(false); // 新建任务窗口
  const [isRawFileModalVisible, setRawFileModalVisible] = useState(false); //  导入raw脚本窗口
  ...
  return (
    <PageContainer
      ...
      extra={[
        ...
        <Button key="2" type="primary" onClick={() => addRawFile()}>
          添加raw脚本
        </Button>,
      ]}
      header={{
    ...
    >
      ...
      {/* rawfile弹窗 */}
      <RawFileModal
        visible={isRawFileModalVisible}
        handleCancel={() => {
          setRawFileModalVisible(false);
          setTimeout(getCrons, 2000);
        }}
      />
    </PageContainer>
  );
